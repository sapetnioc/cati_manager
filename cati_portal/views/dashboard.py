...
@view_config(route_name='dashboard', renderer='templates/home.jinja2',
             permission='cati_portal_valid_user')
def dashboard(request):
    check_maintenance(request)
    dashboards = []
    with user_connect(request) as db:
        with db.cursor() as cur:
            if request.has_permission('cati_portal_user_moderator'):
                items = table_select(db, 'cati_portal', 'identity_email_not_verified', as_dict=True)
                if items:
                    dashboards.append({
                        'item_type': 'user_email_not_verified',
                        'items': items})
                items = table_select(db, 'cati_portal', 'identity_not_validated', as_dict=True)
                if items:
                    dashboards.append({
